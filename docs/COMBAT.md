...
Phase 2 code is committed on the bootstrap branch. The current GitHub-hosted CI rerun remains dependent on the account Actions billing/spending state being unblocked; until then the automated checks are present but cannot be claimed as executed on the Phase 2 head commit.
