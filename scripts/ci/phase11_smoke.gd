...
func _initialize() -> void:
	...
	if not updater_text.contains("24600:24749/udp") or not updater_text.contains("sync_objetos3d.sh") or not updater_text.contains("submodule update --init --recursive") or not updater_text.contains("vendor/Objetos3D"):
		_fail("Phase 11.3 VPS dynamic match/vendored-model deployment contract missing")
		return
