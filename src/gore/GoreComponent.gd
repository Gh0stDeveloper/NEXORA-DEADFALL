...
func _ready() -> void:
	_health = get_node_or_null(health_path); _hitboxes = get_node_or_null(hitboxes_path) as Node3D; _rig = get_node_or_null(rig_path) as Node3D; _wounds = get_node_or_null(wounds_path) as Node3D
	if _health != null and _health.has_signal("health_changed"): _health.connect("health_changed", Callable(self, "_on_health_changed"))

func process_damage_event(event) -> void:
	if event == null or float(event.resolved_amount) <= 0.0: return
	if _health != null and int(event.victim_id) != int(_health.get("entity_id")): return
	_spawn_impact_effects(event, 1.0)
	var body_part := int(event.body_part)
	if not _is_dismemberable(body_part): return
	var accumulated := float(_damage_by_part.get(body_part, 0.0)) + float(event.resolved_amount); _damage_by_part[body_part] = accumulated
	var threshold := _threshold_for(body_part); body_part_damage_changed.emit(body_part, accumulated, threshold)
	if not is_limb_destroyed(body_part) and accumulated >= threshold: _destroy_limb(body_part, event)

func is_limb_destroyed(body_part: int) -> bool: return bool(_destroyed.get(body_part, false))
func get_body_part_damage(body_part: int) -> float: return float(_damage_by_part.get(body_part, 0.0))
...
func get_destroyed_parts() -> Array:
	...
	for body_part in _destroyed.keys():
		if bool(_destroyed[body_part]): result.append(int(body_part))
	result.sort(); return result

func apply_replica_destroyed_parts(parts: Array) -> void:
	for value in parts:
		var body_part := int(value)
		if _is_dismemberable(body_part) and not is_limb_destroyed(body_part): _apply_replica_limb(body_part)
...
func _apply_replica_limb(body_part: int) -> void:
	...
	match body_part:
		DamageEventScript.BodyPart.LEFT_LEG, DamageEventScript.BodyPart.RIGHT_LEG: crawler_required.emit(null)
...
func requires_crawler() -> bool: return is_limb_destroyed(DamageEventScript.BodyPart.LEFT_LEG) or is_limb_destroyed(DamageEventScript.BodyPart.RIGHT_LEG)
...
func _on_health_changed(_current: float, _maximum: float, event) -> void:
	if event != null: process_damage_event(event)
...
func _destroy_limb(body_part: int, event) -> void:
	...
	if manager != null:
		manager.call("spawn_detached_limb", body_part, spawn_transform, Vector3(event.hit_direction)); manager.call("spawn_blood", Vector3(event.hit_position), Vector3(event.hit_direction), 2.2)
		if parent_3d != null:
			var ground_position := parent_3d.global_position; ground_position.y = 0.03; manager.call("spawn_blood_decal", ground_position, 1.15)
...
func _disable_hitbox(part_name: StringName) -> void:
	...
	if shape != null: shape.set_deferred("disabled", true)
func _threshold_for(body_part: int) -> float:
	match body_part:
		...
		_: return INF
func _is_dismemberable(body_part: int) -> bool: return body_part in [DamageEventScript.BodyPart.HEAD, DamageEventScript.BodyPart.LEFT_ARM, DamageEventScript.BodyPart.RIGHT_ARM, DamageEventScript.BodyPart.LEFT_LEG, DamageEventScript.BodyPart.RIGHT_LEG]
